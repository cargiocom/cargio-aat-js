import { CargioAAT } from '../src/cargio-aat'
...
describe('CargioAAT Class tests', () => {
    describe('Valid AAT parameters', () => {
        it('should create a valid CargioAAT instance', async () => {
            const cargioAAT = await CargioAAT.from(version, clientPublicKey, applicationPublicKey, applicationPrivateKey)
            expect(cargioAAT).to.not.be.an.instanceof(TypeError)
            expect(cargioAAT).to.be.an.instanceof(CargioAAT)
            expect(cargioAAT.isValid()).to.equal(true)
        })

        it('should create a valid CargioAAT instance with an already created signature', () => {
            const cargioAAT = new CargioAAT(version, clientPublicKey, applicationPublicKey, applicationSignature)
            expect(cargioAAT).to.not.be.an.instanceof(TypeError)
            expect(cargioAAT).to.be.an.instanceof(CargioAAT);
            expect(cargioAAT.isValid()).to.equal(true);
        })
    ...
    describe('Invalid Hex AAT parameters', () => {
        it('should fail given an invalid hex clientPublicKey', async () => {
            try {
                await CargioAAT.from(version, invalidHexClientPublicKey, applicationPublicKey, applicationPrivateKey)
            } catch (error) {
        ...
        it('should fail given an invalid hex applicationPublicKey', async () => {
            try {
                await CargioAAT.from(version, clientPublicKey, invalidHexApplicationPublicKey, applicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(TypeError)
            }
        ...
        it('should fail given an invalid hex applicationPrivateKey', async () => {
            try {
                await CargioAAT.from(version, clientPublicKey, applicationPublicKey, invalidHexApplicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(TypeError)
            }
        })
        ...
        it('Should fail given an invalid hex Application Signature', () => {
            try {
                new CargioAAT(version, clientPublicKey, applicationPublicKey, invalidHexApplicationSignature);
            } catch (error) {
    ...
    describe('Unsupported or invalid AAT Scenarios', () => {
        it('should fail given an unsupported version', async () => {
            try {
                await CargioAAT.from('1.0.2', clientPublicKey, applicationPublicKey, applicationPrivateKey);
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        it('should fail given an Application Signature that isnt related to the Application Public Key', () => {
            try {
                new CargioAAT(version, clientPublicKey, applicationPublicKey, invalidApplicationSignature);
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
    ...
    describe('Wrong AAT parameters', () => {
        it('should fail given an empty version', async () => {
            try {
                await CargioAAT.from('', clientPublicKey, applicationPublicKey, applicationPrivateKey);
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        ...
        it('should fail given an empty clientPublicKey', async () => {
            try {
                await CargioAAT.from(version, '', applicationPublicKey, applicationPrivateKey);
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        ...
        it('should fail given an empty applicationPublicKey', async () => {
            try {
                await CargioAAT.from(version, clientPublicKey, '', applicationPrivateKey);
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        ...
        it('should fail given an empty applicationPrivateKey', async () => {
            try {
                await CargioAAT.from(version, clientPublicKey, applicationPublicKey, '');
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
